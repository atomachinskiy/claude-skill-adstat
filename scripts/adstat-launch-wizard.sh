#!/usr/bin/env bash
# adstat-launch-wizard.sh — открывает ОТДЕЛЬНОЕ окно терминала с интерактивным
...
# эмулятор на Linux), где он вводит логин/пароль от Adstat.
#
# AI пароль не видит — он остаётся в отдельном окне.
#
# На Windows AI должен звать adstat-launch-wizard.ps1 через PowerShell.

set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
WIZARD="$SCRIPT_DIR/adstat-oauth-setup.sh"
...
EOF
    echo "✅ Открыл Terminal.app с мастером Adstat. Перейди в новое окно."
    ;;
  ...
  Linux|WSL*)
    ...
    echo "✅ Открыл терминал с мастером Adstat."
    ;;

  MINGW*|MSYS*|CYGWIN*)
    # На Windows AI должен предпочесть adstat-launch-wizard.ps1 (native PS).
    ...
    echo "✅ Открыл PowerShell-окно с мастером Adstat (через Git Bash)."
    echo "💡 Если Git Bash не установлен — используй вместо этого скрипта:"
    echo "    powershell -ExecutionPolicy Bypass -File \"$SCRIPT_DIR/adstat-launch-wizard.ps1\""
    ;;
